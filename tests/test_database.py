...
def test_user_operations(client):
    ...
    with app.app_context():
        # Test user creation
        user = UserOperations.create_user("db-test@example.com", VALID_PASSWORD)
        assert user is not None
        assert user.email == "db-test@example.com"

        # Test duplicate email
        duplicate_user = UserOperations.create_user("db-test@example.com", "Password456!")
        assert duplicate_user is None

        # Test user authentication
        auth_user = UserOperations.authenticate_user("db-test@example.com", VALID_PASSWORD)
        assert auth_user is not None
        assert auth_user.id == user.id

        # Test wrong password
        wrong_auth = UserOperations.authenticate_user("db-test@example.com", "WrongPassword123!")
        assert wrong_auth is None

        # Test get user by email
        found_user = UserOperations.get_user_by_email("db-test@example.com")
...
def test_chat_operations(client):
    ...
    with app.app_context():
        # Create test user
        user = UserOperations.create_user("chat@example.com", VALID_PASSWORD)
...
def test_message_operations(client):
    ...
    with app.app_context():
        ...
        user = UserOperations.create_user("msg@example.com", VALID_PASSWORD)
...
def test_settings_operations(client, monkeypatch):
    """Test settings CRUD operations"""
    monkeypatch.setenv("DEFAULT_OLLAMA_HOST", "http://localhost:11434")
    with app.app_context():
        # Create test user
        user = UserOperations.create_user("db-settings@example.com", VALID_PASSWORD)
        ...
        updated_settings = SettingsOperations.update_ollama_host(user.id, "http://other-host:11434")
        assert updated_settings.ollama_host == "http://other-host:11434"

        # Test get OLLAMA host
        host = SettingsOperations.get_ollama_host(user.id)
        assert host == "http://other-host:11434"
